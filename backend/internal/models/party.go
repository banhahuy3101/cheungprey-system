...
type PartyFinance struct {
	...
	Attachments       []PartyFile `json:"attachments,omitempty"`
}

const (
	FinanceTypeDonation         = "Donation"
	FinanceTypeEventFundraising = "Event Fundraising"
	FinanceTypeExpense          = "Expense"
)

var FinanceTransactionTypes = []string{
	FinanceTypeDonation,
	FinanceTypeEventFundraising,
	FinanceTypeExpense,
}

var FinancePaymentMethods = []string{
	"Bakong/KHQR",
	"Cash",
	"Bank Transfer",
	"Other",
...
func IsFinanceExpense(transactionType string) bool {
	return transactionType == FinanceTypeExpense
}
...
type FinanceListResult struct {
	Finances []PartyFinance `json:"finances"`
...
type FinanceSummary struct {
	...
	Budgets      []FinanceBudget        `json:"budgets,omitempty"`
}

type FinanceZoneBreakdown struct {
	ZoneCode     string  `json:"zone_code"`
	ZoneNameKh   string  `json:"zone_name_kh"`
	ZoneType     string  `json:"zone_type"`
	TotalIncome  float64 `json:"total_income"`
	TotalExpense float64 `json:"total_expense"`
	Balance      float64 `json:"balance"`
